...
export default class StartFit extends Component {
  ...
  updateTime = (time) => {
    let round = 0;
    if (
      this.state.title !== "READY TO START" &&
      this.state.title !== "REST 10 SECONDS"
    ) {
      round = 1;
    }
    const docFitness = firebaseApp
      .firestore()
      .collection("users")
      ...
      .doc("records")
      .update({
        workoutSeconds: this.state.workoutSeconds + time,
        completedRounds: this.state.completedRounds + round,
      });
        ...
        <View style={styles.header}>
          ...
          >
            <Text
              style={[
                styles.buttonText,
                { fontSize: 40, textAlign: "center", marginTop: 0 },
              ]}
            >
              {this.state.timer}
            </Text>
